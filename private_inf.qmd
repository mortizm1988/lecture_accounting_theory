...
Let $k^*$ be the value of $i$ that maximizes the principal's problem. This implies that $x_i=0$ for $i=1,...,k^*-1$ and $x_i=x_{\max}$ for all $i=k^*,...,N$. Since $z_1=m_1 x_1+\theta$, and then $z_{i}-z_{i-1}=m_i(x_i-x_{i-1})$ for $i=2,...,N$, the transfer policy is


$$
z_i = 
\begin{cases} 
    \theta & \text{for } i=1,...,k^*-1, \\
    m_{k^*} x_{\max} + \theta & \text{for } i=k^*,...,N
\end{cases}
$$
