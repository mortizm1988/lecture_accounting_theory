...
Solving the ICC in @eq-agent_problem can be complex. However, under certain conditions, the agent's optimal effort can be determined by the **first-order condition** of the ICC. Assuming the agent’s optimal effort is interior, the first-order condition is:
...
If $x=e+a_1$ and $y=x+a_2$, where $a_1$ and $a_2$ are independent random variables, then $x$ is a **sufficient statistic** for $y$ with respect to $e$. In other words, $y$ does not add any information about $e$ that is not already contained in $x$.
:::
...
Therefore, the Certainty Equivalent (CE)  of the agent's expected utility is:
$$
CE = \underbrace{t + se}_{\text{Expected Wage}} - \underbrace{\frac{1}{2}ce^{2}}_{\text{Cost of Effort}} - \underbrace{s^{2}\eta\frac{\sigma^{2}}{2}}_{\text{Risk Premium}}
$$
...
```
We can also decompose the expected compensation into its components: reservation utility, cost of effort, and risk premium.

```{python}
# Decomposition of the expected compensation
cost_of_effort = 0.5 * c * effort**2
risk_premium = 0.5 * eta * s**2 * sigma**2
reservation_utility = np.full_like(sigma, w)

plt.stackplot(sigma, reservation_utility, cost_of_effort, risk_premium, 
              labels=['Reservation Utility', 'Cost of Effort', 'Risk Premium'], 
              alpha=0.6)
plt.plot(sigma, total, label='Total Compensation', color='black', linestyle='--')
plt.legend(loc='center right')
plt.xlabel('Value of $\sigma$')
plt.ylabel('Value')
plt.title('Components of Expected Compensation')
plt.show()
```
